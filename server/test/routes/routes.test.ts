import { utilFunction } from '../../src/utils/routes.util';

describe('utilFunction', () => {
  it('shuld retrun object with isMoreThenFive property equal to false', () => {
    const { isMoreThenFive } = utilFunction(4);
    expect(isMoreThenFive).toBe(false);
  });
  it('shuld retrun object with isMoreThenFive property equal to true', () => {
    const { isMoreThenFive } = utilFunction(6);
    expect(isMoreThenFive).toBe(true);
  });
});
